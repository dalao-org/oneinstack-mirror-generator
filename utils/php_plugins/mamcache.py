...
def make_cache() -> list:
    ...
    while tried < MAX_TRIES:
        try:
            ...
            for a in soup.find_all("a"):
                if a.text.startswith("memcache-") and a.text.endswith(".tgz") and "rc" not in a.text.lower():
                    resource_list.append({
                        "version": a.text.replace("memcache-", "").replace(".tgz", ""),
    ...
    print("pcel.php.net is down. Failed to fetch memcache versions.")
    return []
